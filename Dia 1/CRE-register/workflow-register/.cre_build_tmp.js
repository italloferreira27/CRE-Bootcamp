...
var configSchema = exports_external.object({
  ...
  registerUsingCRESolAddress: exports_external.string(),
  gasLimit: exports_external.string()
});
var network282;
var evmClient;
function onACada5Min(runtime2, payload) {
  if (!payload.scheduledExecutionTime)
    throw new Error("Missing scheduledExecutionTime");
  runtime2.log("A cada 5 min triggered at " + payload.scheduledExecutionTime);
  const reportPayload = encodeAbiParameters(parseAbiParameters("string myinfo"), ["$Comprem Bitcoin!$"]);
  ...
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.registerUsingCRESolAddress,
    report: report2,
  ...
  runtime2.log(`RegisterUsingCRE.sol.registerSet tx: ${bytesToHex3(writeResult.txHash || new Uint8Array(32))}`);
  return "ok";
}
function initWorkflow(config) {
  network282 = getNetwork({
    chainFamily: "evm",
  ...
  evmClient = new ClientCapability(network282.chainSelector.selector);
